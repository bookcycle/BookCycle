import { createRequest, listMyTransactionsForBook, acceptRequest } from "../services/transaction.service.js";
...
export async function listTransactionsCtrl(req, res, next) {
  try {
    ...
    const rows = await listMyTransactionsForBook({ userId: req.user.id, bookId: book_id });
