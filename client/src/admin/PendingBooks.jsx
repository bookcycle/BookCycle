...
export default function PendingBooks() {
  ...
  if (err) return <div className="text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded-lg">{err}</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Pending Books</h1>
          <p className="text-sm text-slate-500">Smart, minimal review queue.</p>
        </div>
      ...
      <div className="space-y-3">
        {filtered.map((b) => {
          const busyApprove = acting === `${b._id}:accepted`;
          const busyReject = acting === `${b._id}:rejected`;
          ...
          return (
            ...
            >
              {/* subtle accent rail */}
              <div className="absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-violet-400 to-sky-400" />
              ...
              <div className="flex gap-4 p-4 pl-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-sky-100 text-sky-700 text-xs font-medium px-2.5 py-1">
                      Pending
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      <button
                        onClick={() => {
                          const ok = window.confirm(
                            `Reject “${b.title}” by ${b.author}?`
                          );
                          if (ok) changeStatus(b._id, "rejected");
                        }}
                        disabled={busyReject}
                        className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50"
                      >
                        {busyReject ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg bg-violet-600 text-white hover:bg-violet-700 disabled:opacity-50"
                      >
                        {busyApprove ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                ...
                <div className="border-t border-slate-200">
                  ...
                  <div className="px-4 pb-4">
                    <div className="flex items-start gap-2 text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded-lg p-3">
                      ...
                      <p>
                        Approving moves this book to the public catalogue. You can
                        change its status later from the book details page.
                      </p>
                    </div>
                  </div>
                </div>
...
function Pill({ children }) {
  return (
    <span className="inline-flex items-center text-xs px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 border border-violet-100">
      {children}
    </span>
  );
}
