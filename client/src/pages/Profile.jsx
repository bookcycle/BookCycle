...
const dispatch = useDispatch();
  ...
  // ProtectedRoute নিশ্চিত করেছে টোকেন আছে, তাই শুধু ডেটা-লোডিং রাখছি
  ...
  useEffect(() => {
    ...
    async function fetchMe() {
      if (storeUser) { setLoading(false); return; }
      setLoading(true);
      try {
        const { user } = await api("/auth/me", { token: savedToken });
        if (!ignore) {
          dispatch(setCredentials({ user, token: savedToken }));
          setLoading(false);
        }
      } catch (e) {
        if (!ignore) { setError(e.message || "Failed to load profile"); setLoading(false); }
      }
    }
    fetchMe();
    return () => { ignore = true; };
  }, [dispatch, savedToken, storeUser]);
  ...
  return (
    <div
      ...
      style={{
        // soft linen-like bg using subtle pattern
        background:
          "linear-gradient(180deg, #F7F5F2 0%, #F3F1EC 100%)",
      }}
    >

      {/* Main */}
      <main className="max-w-6xl mx-auto px-4 py-10 md:py-14">
        {/* Hero: Identity + Single CTA */}
        <section className="grid lg:grid-cols-[1.3fr_0.7fr] gap-6">
          {/* Identity card (paper feel) */}
          <PaperCard className="p-6">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="relative">
                {user?.avatarUrl ? (
                  <img
                    src={user.avatarUrl}
                    alt="avatar"
                    className="w-28 h-28 md:w-32 md:h-32 rounded-2xl object-cover ring-4 ring-white shadow"
                  />
                ) : (
                  <div className="w-28 h-28 md:w-32 md:h-32 rounded-2xl bg-[#1F2421] text-[#F7F5F2] flex items-center justify-center text-3xl font-semibold ring-4 ring-white shadow">
                    {initials}
                  </div>
                )}
                <button
                  onClick={() => alert("Edit avatar (coming soon)")}
                  className="absolute -bottom-2 -right-2 px-3 py-1.5 rounded-xl bg-white border border-[#1F2421]/10 shadow hover:shadow-md text-sm"
                  title="Edit avatar"
                >
                  <FaUserEdit />
                </button>
              ...
              <div className="flex-1">
                ...
                <p className="text-[#1F2421]/70 mt-1 font-medium">{user?.email}</p>

                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <Pill>Joined {joined}</Pill>
                  <Pill>Community Reader</Pill>
                  <Pill>PassTheBook</Pill>
                </div>
          ...
          {/* Single standout CTA (quiet, elegant) */}
          <PaperCard className="p-6 flex flex-col justify-between">
            <div>
              ...
              <p className="text-[#1F2421]/70 mt-1">
                Borrow • exchange • giveaway — grow a culture of shared reading.
              </p>
            </div>
            <div>
              <button
              ...
              >
                <FaCloudUploadAlt /> Upload Books
              </button>
              <p className="text-xs text-[#1F2421]/60 mt-2">
                Add a cover, condition & choose listing type.
              </p>
            </div>
          </PaperCard>
        </section>
        ...
        <section className="grid lg:grid-cols-3 gap-6 mt-8">
          <PaperCard className="p-6 lg:col-span-2">
            ...
            <ul className="mt-4 space-y-3">
              {[
                "Borrowed “Atomic Habits” from Ayesha",
                "Saved “Deep Work” to My Shelf",
                "Gave away “The Alchemist”",
              ].map((t, i) => (
                <li
                  key={i}
                  className="p-3 rounded-xl bg-white border border-[#1F2421]/10"
                >
                  {t}
                </li>
              ))}
          ...
          <PaperCard className="p-6">
            ...
            <div className="mt-4 space-y-2">
              <StatRow label="Books Borrowed" value="12" />
              <StatRow label="My Shelf" value="28" />
              <StatRow label="Favorites" value="9" />
            </div>
          </PaperCard>
        </section>
      </main>
...
function Pill({ children }) {
  return (
    <span className="px-3 py-1 rounded-full bg-white border border-[#1F2421]/10 text-sm">
      {children}
    </span>
  );
}
...
function Skeleton() {
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(180deg,#F7F5F2 0%,#F3F1EC 100%)" }}>
